...
$load_prettypop = 1;
$load_import_js = 1;
$_page_title = 'Import from youtube user';
include('header.php');
		...
		if ( !xml_parse($xml_parser, $data, TRUE) ) 
		{
			...
			else
			{
				if (xml_get_error_code($xml_parser) == XML_ERROR_INVALID_TOKEN && xml_get_current_line_number($xml_parser) == 1 && strlen($data) > 0)
				{
					...
					{
						die(pm_alert_error('User not found.'));
					}
					die(pm_alert_error(sprintf('<strong>API response:</strong> %s', htmlentities($data))));
				}
	   ...
	   if ( $fp = @fopen($file, "r") ) 
	   {
	      while ($data = fread($fp, 4096)) 
	      {
	        if ( !xml_parse($xml_parser, $data, feof($fp)) ) 
	        {
				...
				{
					if (xml_get_error_code($xml_parser) == XML_ERROR_INVALID_TOKEN && strlen($data) > 0)
					{
						if ($data == 'Resource not found')
						{
							die(pm_alert_error('User not found.'));
...
Simply type the desired YouTube username and start importing.</p>
	...
	{
		echo pm_alert_info('Import playlists, favorites and uploaded videos from any Youtube.com user.<br /> <small>Please note that <strong>private</strong> playlists will appear as being empty.</small>');
	}
	?>
...
<input name="username" type="text"  value="<?php if($_POST['username'] != '') echo $_POST['username']; elseif($_GET['username'] != '') echo $_GET['username']; else echo 'Youtube username';?>" placeholder="Youtube username" class="span5" />
		...
		<tbody>
			...
			<tr id="row-subscription-<?php echo $sub['sub_id']; ?>">
				<td>
					...
					<img src="<?php echo $sub['profile_avatar_url'];?>" width="50" height="50" />
					<?php endif; ?>
				</td>
				<td>
					<?php 
					...
					<br />
					<small>Subscribed by <a href="<?php echo _URL .'/profile.php?u='. $sub['sub_username'];?>" target="_blank"><?php echo $sub['sub_username'];?></a></small>
				</td>
...
if(($action == 'search' || $action == 'favorites' || $action == 'playlists') && ($username != ''))
{
	...
	$yt_api_url = "http://gdata.youtube.com/feeds/api/users/".$username."/playlists?start-index=1&max-results=".$import_results;

	$youtube = array();
	$youtube_index = "";
	$youtube_array_counter = 0;
	grab_and_parse_xml($yt_api_url);
...
?>
  ...
  <div id="import_user">
	...
	{
		case 'search':
			$yt_api_url = "http://gdata.youtube.com/feeds/api/users/". $username ."/uploads?start-index=". $start_from ."&max-results=". $import_results ."&v=2";
			echo "<h2 class='sub-heading'><strong>Latest Uploads</strong></h2>";
		break;
		case 'favorites':
			$yt_api_url = "http://gdata.youtube.com/feeds/api/users/". $username ."/favorites?start-index=". $start_from ."&max-results=". $import_results ."&v=2";
		...
		case 'playlists':
			...
			$yt_api_url = "http://gdata.youtube.com/feeds/api/playlists/". $play_id ."?start-index=". $start_from ."&max-results=". $import_results ."&v=2";
				...
				?>
				 <div class="pagination pagination-centered">
				  <?php
					...
					$ext .= ($_GET['sub_id'] != '') ? '&sub_id='. $_GET['sub_id'] : '';
