...
<link rel="shortcut icon" type="image/ico" href="admin/img/favicon.ico" />
...
<body>
<style type="text/css" />
body{font-family:"Open Sans",Arial,Helvetica,sans-serif;background-image:none;background-color:#0060a2}
...
ul li .label{position:absolute;right:0;top:2px;padding:3px 4px;border-radius:30px}
...
.icon-file,.icon-folder-open{opacity:.5}
...
<header>
<img src="admin/img/install-logo.png" width="179" height="34" align="PHP Melody" />
</header>
...
case 1:
		...
		if( ! file_exists('config.php')) { 
			$error = 1;
			echo "not found. <span class=\"label label-warning pull-right\"><i class='icon-exclamation-sign icon-white'></i></span>";
		}
		else {
			echo "<span class=\"label label-success pull-right\"><i class='icon-ok icon-white'></i></span>";
		}
		echo "</li>";
		...
		if( ! $connection )  {
			$error = 1;
			echo "<span class=\"warn\">ERROR: ".mysql_error()."</span> <span class=\"label label-warning pull-right\"><i class='icon-exclamation-sign icon-white'></i></span>";
			}
			else {
				echo "<span class=\"label label-success pull-right\"><i class='icon-ok icon-white'></i></span>";
			}
	...
	if ( ! is_writable(ABSPATH . $path) ) {
		...
		echo '<li><i class="'. $icon .'"></i> 	<strong>'.$path.'</strong> must be writable. Set CHMOD 0777.</li>';
			...
			echo "<ul class=\"file-check\">";
					...
					pm_file_writtable("admin/tmp.pm");
					pm_file_writtable("admin/temp/embedparams.xml");
					pm_file_writtable("admin/temp/");
		...
		if( $result ) {	
			$error = 1;
			echo "<p class=\"alert alert-warning border-radius4\">The installation cannot continue because existing PHP Melody MySQL tables were detected in the <strong>".$db_name."</strong> database. If this is a re-installation, empty the <strong>".$db_name."</strong> MySQL database and click '<strong>Retry</strong>'.<br /><span class=\"label label-warning\">Remember to backup your MySQL data before deleting anything important!</span></p>";
		}
		if ( _CUSTOMER_ID == '' || _CUSTOMER_ID == 'YOUR_CUSTOMER_ID' ){
			$error = 1;
			echo "<p class=\"alert alert-warning border-radius4\">The installation cannot continue because your <strong>Customer ID</strong> is not defined in <strong>config.php</strong>. <br /> Update the following <strong>config.php</strong> line:<br /> <code>define('_CUSTOMER_ID', 'YOUR_CUSTOMER_ID');</code></p>";
...
case 2:
		...
		?>
  <p class="alert alert-warning border-radius4" id="please_wait"><img src="admin/img/ico-loading.gif" width="16" height="16" border="0" align="absmiddle" /> Please wait while the machines do all the work...</p>
  <?php
		...
		//open the .sql file
		  if ( !$file = @fopen($filename,"rt")){
			...
			echo "<br />The script was stopped at this line : <strong>".__LINE__."</strong></p>";
			$error = 1;
		  }
...
//			If the script ended here, it means that the file pointer could not be set at $_REQUEST["pointer"];
			$error = 1;
			echo "<br /><p class=\"alert alert-warning\">The script was stopped at this line : <strong>".__LINE__."</strong></p>";
		  }
			  ...
			  echo "<h1>PHP Melody is installed and ready to go!</h1>";
				if((!@unlink($filename)) || (!@unlink("install.php"))){
				  ...
				  echo "<p class=\"alert alert-warning border-radius4\">Before anything else, please remove the following files:<br />
						<i class=\"icon-file\"></i>  <strong>install.php</strong><br />
						<i class=\"icon-file\"></i>  <strong>".$filename."</strong></p>";
				  echo "</p>";
				}
			  echo "<p class=\"lead\">You can <a href=\""._URL."/admin/login.php\" target=\"_blank\">login</a> as an Administrator with username '<strong>admin</strong>' and password '<strong>admin</strong>'.";
			  echo "</p>";
			  echo "<br /><div align=\"center\"><a href=\""._URL."/admin/login.php\" target=\"_blank\" class=\"btn btn-blue\">Take me to the admin panel &rarr;</a></div>";
			  echo "";
//			  $error = 1;
			}
			else
...
?>
  <div id="footer">
  <a href="http://www.phpsugar.com/" title="Powered by PHPSUGAR.com" target="_blank"><img src="http://www.phpsugar.com/updates/phpsugar.gif&license=<?php echo _CUSTOMER_ID;?>" border="0" alt="Powered by PHPSUGAR.com" /></a><br />
  <p>Copyright &copy; <?php echo date('Y'); ?><br />
  Need help? <a href="http://www.phpsugar.com/support.html" target="_blank">Click here</a>
  </p>
  </div>
</div>
</body>
